...
public record PathForStampKey(int nid, int stampNid) implements FeatureKey.VersionFeature.Stamp.Path {
    public PathForStampKey() {
    ...
    @Decoder
    public static PathForStampKey decode(DecoderInput in) {
        return switch (Encodable.checkVersion(in)) {
            default -> new PathForStampKey(in.readNid(), in.readNid());
        };
    }
}
