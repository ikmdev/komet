...
public record AuthorForStampKey(int nid, int stampNid) implements FeatureKey.VersionFeature.Stamp.Author {
    public AuthorForStampKey() {
    ...
    @Decoder
    public static AuthorForStampKey decode(DecoderInput in) {
        return switch (Encodable.checkVersion(in)) {
            default -> new AuthorForStampKey(in.readNid(), in.readNid());
        };
    }
}
