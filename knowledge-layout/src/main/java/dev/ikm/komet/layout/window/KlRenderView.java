 ...
 * the scene and its root {@link Node}, and inherits behavior for managing preferences
 ...
 */
public non-sealed interface KlRenderView extends KlView<Scene> {
    ...
    KlTopView topView();
    /**
     * Retrieves the root node of the {@code KlRenderView}.
     * The root node serves as the top-most parent of all visual elements within the {@code KlRenderView}.
     *
     * @return The {@link Node} object representing the root node of the {@code KlRenderView}.
     */
    Parent fxRoot();

    <FX extends Region> void setKlRootArea(KlArea<FX> rootArea);

    <FX extends Region> KlArea<FX> getKlRootArea();

    KnowledgeLayout getMasterLayout();

    static <KL extends KlRenderView> KL restore(KometPreferences preferences) {
        return KlView.restore(preferences);
    }

    non-sealed interface Factory<KL extends KlRenderView> extends KlView.Factory<Scene, KL>  {
