...
public class EdgeResizeHandle extends AbstractResizeHandle {
    ...
    /** The line shape representing this edge handle. */
    private final Line line;

    /** The color to use for the handle (visible in debug mode, transparent otherwise). */
    private final Color debugColor;

    /** The width of the hit area in pixels, determining how easy it is to grab the handle. */
    private final double hitArea;
    ...
    public EdgeResizeHandle(ResizeDirection direction, double hitArea, boolean debugMode) {
        ...
        this.debugColor = debugMode ? Color.RED : Color.TRANSPARENT;
        this.line = createHitAreaLine();
    }
     ...
     */
    private Line createHitAreaLine() {
        Line line = new Line();
        line.setStrokeWidth(hitArea);
        line.setStroke(debugColor);
        line.setUserData(getCursor());
        return line;
    }
