...
public class ProcessElkOwlResultsTask extends TrackingCallable<ClassifierResults> {
	...
	private ClassifierResults collectResults(ImmutableIntList classificationConceptSet, Transaction updateTransaction) {
		...
		classificationConceptSet.primitiveParallelStream().forEach((conceptNid) -> {
			completedUnitOfWork();
			OWLClass node = axiomData.nidConceptMap.get(conceptNid);
			if (node == null) {
				// TODO emays Need to resolve this with inactive in Extract
				...
				Set<OWLClass> equivalentConcepts = reasoner.getEquivalentClasses(node).getEntities();
...
//						LOG.info("Eq: " + equivalentConcept + " " + equivalentConcept.getIRI());
						int equivalentNid = Integer.parseInt(equivalentConcept.getIRI().getShortForm());
