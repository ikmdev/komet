...
public class CountingEvictor<E> extends RecencyEvictor<E> {
	...
	public Iterator<E> evict(final Predicate<E> retain) {
		return Stream.concat(StreamSupport.stream(Spliterators.spliteratorUnknownSize(super.evict(retain), Spliterator.ORDERED), false),
				StreamSupport.stream(Spliterators.spliteratorUnknownSize(immediatelyEvicted_.evict(retain), Spliterator.ORDERED), false)).iterator();
	}
