...
public class ClassTaxonomyMemberComparisonTest {
	...
	static class Manifest implements TestManifest<TestInput> {
		...
		public Manifest(final ClassTaxonomyProvider classTaxonomyProvider) {
			this.classTaxonomyProvider = classTaxonomyProvider;
		}

		@Override
		public String getName() {
			return classTaxonomyProvider.toString();
		}
	...
	static final ClassTaxonomyProvider REASONER_TAXONOMY = new ClassTaxonomyProvider() {
		@Override
		public Taxonomy<ElkClass> getTaxonomy(final String resource)
				throws ElkException {
	...
	public ClassTaxonomyMemberComparisonTest(final Manifest manifest) {
		this.classTaxonomyProvider_ = manifest.classTaxonomyProvider;
	}

	@Test
	public void testNodeLookup() throws ElkException {
		final Taxonomy<ElkClass> taxonomy = classTaxonomyProvider_.getTaxonomy(
				"taxonomy_member_comparison/class_node_lookup.owl");

		// IRI-s that look different, but are the same point to the same node

		final ElkClass sameA1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/same#A"));

		final ElkClass sameA2 = OBJECT_FACTORY
				.getClass(
		...
		final ElkClass sameA3 = OBJECT_FACTORY
				.getClass(
						new ElkAbbreviatedIri(
		...
		final ElkClass differentA1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/different#A"));

		final ElkClass differentA2 = OBJECT_FACTORY
				.getClass(new ElkAbbreviatedIri(
						new ElkPrefixImpl("different:",
								new ElkFullIri(
										"http://example.org/different#")),
						"A"));

		final TaxonomyNode<ElkClass> nodeDifferentA1 = taxonomy
				.getNode(differentA1);
		final TaxonomyNode<ElkClass> nodeDifferentA2 = taxonomy
				.getNode(differentA2);

		assertNotSame(nodeSameA1, nodeDifferentA1);
		assertNotSame(nodeSameA1, nodeDifferentA2);

		final ElkClass sameC1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/same#C"));

		final ElkClass sameC2 = OBJECT_FACTORY
				.getClass(
						new ElkAbbreviatedIri(
								new ElkPrefixImpl("same1:",
										new ElkFullIri(
		...
		final ElkClass sameC3 = OBJECT_FACTORY
				.getClass(
						new ElkAbbreviatedIri(
								new ElkPrefixImpl("same2:",
										new ElkFullIri(
	...
	public void testMemberSet() throws ElkException {
		final Taxonomy<ElkClass> taxonomy = classTaxonomyProvider_
				.getTaxonomy("taxonomy_member_comparison/class_member_set.owl");
		...
		final ElkClass sameB1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/same#B"));
		final ElkClass sameB2 = OBJECT_FACTORY
				.getClass(
						new ElkAbbreviatedIri(
								new ElkPrefixImpl("same1:",
										new ElkFullIri(
												"http://example.org/same#")),
								"B"));
		final ElkClass sameB3 = OBJECT_FACTORY
				.getClass(
						new ElkAbbreviatedIri(
								new ElkPrefixImpl("same2:",
										new ElkFullIri(
												"http://example.org/same#")),
								"B"));
		final ElkClass differentB1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/different#B"));
		final ElkClass differentB2 = OBJECT_FACTORY
				.getClass(new ElkAbbreviatedIri(
						new ElkPrefixImpl("different:",
								new ElkFullIri(
										"http://example.org/different#")),
						...
						new ElkAbbreviatedIri(
								new ElkPrefixImpl("same2:",
										new ElkFullIri(
												"http://example.org/same#")),
								"C"));
		final ElkClass differentC1 = OBJECT_FACTORY
				.getClass(new ElkFullIri("http://example.org/different#C"));
		final ElkClass differentC2 = OBJECT_FACTORY
				.getClass(new ElkAbbreviatedIri(
						new ElkPrefixImpl("different:",
								new ElkFullIri(
										"http://example.org/different#")),
	...
	public void testRelatedNodes() throws ElkException {
		final Taxonomy<ElkClass> taxonomy = classTaxonomyProvider_.getTaxonomy(
				"taxonomy_member_comparison/class_related_nodes.owl");
		...
		final Set<? extends TaxonomyNode<ElkClass>> superA = nodeA
				.getDirectSuperNodes();
		...
		final Set<? extends TaxonomyNode<ElkClass>> subA = nodeA
				.getDirectSubNodes();
